...
export default function MfaSetup() {
  ...
  const handleSetup = () => {
    setupMutation.mutate(undefined, {
      ...
      onError: () => {
        toast({ variant: "destructive", title: "Setup Unavailable", description: "MFA requires a real TOTP provider before it can be enabled." });
      },
    });
  };
  ...
  const handleVerify = (values: z.infer<typeof verifySchema>) => {
    verifyMutation.mutate(
      { data: values },
      {
        ...
        onError: () => {
          toast({ variant: "destructive", title: "Verification Unavailable", description: "MFA verification is not configured yet." });
        },
      }
    );
  };
