...
export default function SuperAdminTenants() {
  ...
  const [form, setForm] = useState({ name: "", domain: "", adminEmail: "", industry: "", country: "UAE", employeeCount: "" });
  ...
  const deleteTenant = useMutation({
    mutationFn: async (id: number) => {
      const response = await fetch(`/api/tenants/${id}`, { method: "DELETE", credentials: "include" });
  ...
  async function handleCreate() {
    try {
      ...
      setShowCreate(false);
      setForm({ name: "", domain: "", adminEmail: "", industry: "", country: "UAE", employeeCount: "" });
  ...
  async function handleDelete(id: number, name: string) {
    if (!window.confirm(`Delete ${name}? This will remove its users and tenant data.`)) return;
    ...
    <div className="space-y-5">
      ...
      <div className="bg-card/80 border border-border rounded-xl overflow-hidden">
        <div className="grid grid-cols-[2fr_2fr_1fr_1fr_auto] text-xs text-muted-foreground uppercase tracking-wider px-5 py-3 border-b border-border bg-muted/20">
          <div>Organization</div><div>Domain</div><div>Status</div><div>Employees</div><div>Actions</div>
        </div>
        {isLoading ? (
              ...
              <motion.div key={t.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                ...
                <div className="flex gap-2">
                  {editingId === t.id ? (
                    ...
                    <Button size="sm" variant="ghost" className="h-6 text-xs px-2" onClick={() => { setEditingId(t.id); setEditForm({ status: t.status }); }}>Edit</Button>
                  )}
                  <Button
                    size="sm"
                    variant="ghost"
