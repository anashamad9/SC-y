  ...
  return (
    <div className="bg-card/80 border border-border rounded-xl p-4 flex items-center justify-between">
      <div>
        ...
        <div className={`text-xs mt-0.5 ${isHealthy || isConfigured ? "text-emerald-400" : isNotConfigured ? "text-yellow-400" : "text-red-400"}`}>
          {status}
        </div>
      </div>
      <div className="text-right">
        {latency !== undefined && <div className="text-xs text-muted-foreground">{latency}ms</div>}
        <div className={`w-2.5 h-2.5 rounded-full mt-1 ml-auto ${isHealthy || isConfigured ? "bg-emerald-500" : isNotConfigured ? "bg-yellow-500" : "bg-red-500"} animate-pulse`} />
      </div>
    </div>
  );
}
