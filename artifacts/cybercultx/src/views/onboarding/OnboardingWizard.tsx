...
import { useListAssessments, useGetAssessment, useSubmitAssessment, useGetMyScores, useGetLearningPath, useListDepartments } from "@workspace/api-client-react";
...
const STEPS = [
  ...
  { id: 3, label: "Cyber Behavior", icon: "◆" },
  { id: 4, label: "Risk Profile", icon: "◎" },
  { id: 5, label: "Learning Path", icon: "◇" },
];
...
// Inlined mini assessment for wizard steps 2 & 3
...
function RiskProfileStep({ onContinue }: { onContinue: () => void }) {
  ...
  if (!revealed) {
    return (
      <div className="text-center space-y-6 py-4">
        ...
        <div>
          <div className="text-lg font-semibold mb-1">Analyzing Your Behavioral Fingerprint</div>
          <div className="text-sm text-muted-foreground">Computing psychometric profile, risk tolerance, and cultural alignment…</div>
        </div>
  ...
  const cciColor = !scores ? "#6b7280" : scores.cciScore > 65 ? "#22c55e" : scores.cciScore > 50 ? "#f97316" : "#ef4444";

  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      <div className="grid grid-cols-2 gap-4">
        {[
          { label: "Human Risk Score", value: scores?.humanRiskScore ?? 65, color: hrsColor, desc: scores?.riskCategory ?? "Medium Risk" },
          { label: "Culture Index (CCI)", value: scores?.cciScore ?? 52, color: cciColor, desc: "Cultural Alignment" },
        ].map(s => (
      ...
      <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 text-sm text-muted-foreground">
        Your scores are computed from your psychometric profile and behavioral assessment. They will update as you complete more training.
      </div>
...
export default function OnboardingWizard({ user, onComplete }: Props) {
  ...
  const [psychoAssessmentId, setPsychoAssessmentId] = useState<number | null>(null);
  const [cyberAssessmentId, setCyberAssessmentId] = useState<number | null>(null);
  ...
  const cyberAssessment = assessments?.find((a: any) => a.type === "cyber_behavior");
  ...
  const stepContent: Record<number, React.ReactNode> = {
    ...
    3: cyberAssessment ? (
      <AssessmentStep assessmentId={cyberAssessment.id} onComplete={() => setStep(4)} />
    ) : (
      <div className="text-center py-8 text-muted-foreground">Loading assessment…</div>
    ),
    4: <RiskProfileStep onContinue={() => setStep(5)} />,
    5: <LearningPathStep user={user} onComplete={onComplete} />,
  };

  const stepTitles: Record<number, string> = {
    1: "Set Up Your Operative Profile",
    2: "Psychometric Assessment",
    3: "Cyber Behavior Assessment",
    4: "Your Risk Profile",
  ...
  const stepSubtitles: Record<number, string> = {
    ...
    2: "This 8-category assessment maps your behavioral tendencies",
    3: "How would you respond to real-world cyber threats?",
    4: "Based on your assessments, we've computed your risk fingerprint",
    5: "Your recommended security modules based on your risk profile",
  };
