...
function PortalContent({ role, activeKey, stats, departments }: {
  ...
  const { t } = useTranslation();
  const metricCards = [
    { label: t("employee.humanRiskScore"), value: "73", unit: "/100", color: "text-orange-400", trend: "-5 this week" },
    { label: t("employee.cultureIndex"), value: "68", unit: "/100", color: "text-primary", trend: "+3 this month" },
    { label: t("executive.avgCompliance"), value: "81", unit: "%", color: "text-emerald-400", trend: "+8 this quarter" },
    { label: t("common.threatExposure"), value: "22", unit: "%", color: "text-red-400", trend: "-4 this week" },
  ];

  const recentActivity = [
    { action: "Phishing simulation completed", time: "2 hours ago", type: "sim" },
    { action: "Psychometric assessment submitted", time: "5 hours ago", type: "assessment" },
    { action: "Password Security module completed", time: "1 day ago", type: "course" },
    { action: "Risk score updated", time: "2 days ago", type: "risk" },
    { action: "Achievement unlocked: Quick Learner", time: "3 days ago", type: "achievement" },
  ];
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Real-time security culture intelligence</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {metricCards.map((card) => (
            ...
            >
              <div className="text-xs text-muted-foreground mb-3 uppercase tracking-wider">{card.label}</div>
              <div className={`text-3xl font-bold mb-1 ${card.color}`}>
                {card.value}<span className="text-base text-muted-foreground">{card.unit}</span>
              </div>
              <div className="text-xs text-muted-foreground">{card.trend}</div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="bg-card/80 border border-border rounded-xl p-5">
          <h3 className="text-sm font-semibold mb-4 uppercase tracking-wider text-muted-foreground">{t("common.recentActivity")}</h3>
